...
@interface StuJobDetailViewController ()<UITableViewDataSource, UITableViewDelegate, UIAlertViewDelegate>
...
@property(nonatomic, strong) UILabel *attentionContentL4;
...
//*****************************************************************
@property(nonatomic, strong) StuArrangeApplyCommandView *applyView;


@end

@implementation StuJobDetailViewController

- (StuArrangeApplyCommandView *)applyView
{
    if (_applyView == nil) {
        _applyView = [[StuArrangeApplyCommandView alloc]init];
    }
    return _applyView;
}
...
- (void)commandTextViewDismissKeyboard
{
    [self.applyView.commandL
...
- (UIButton *)companyInfoBtn
{
    ...
    [_companyInfoBtn setTitle:@"公司信息" forState:UIControlStateNormal];
...
{
    if (_jobDescImg == nil) {
        _jobDescImg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"wx_jb_002"]];
    }
    return _jobDescImg;
}
...
- (UIImageView *)jobRequreImg
{
    if (_jobRequreImg == nil) {
        _jobRequreImg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"wx_jb_006"]];
    }
...
{
    if (_arrangeImg == nil) {
        _arrangeImg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"wx_jb_005"]];
    }
    return _arrangeImg;
}
...
- (UIImageView *)attentionImg
{
    if (_attentionImg == nil) {
        _attentionImg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"wx_37"]];
    }
...
- (UILabel *)attentionContentL4
{
    if (_attentionContentL4 == nil) {
        _attentionContentL4 = [[UILabel alloc]init];
    }
    [_attentionContentL4 setTextColor:textColor];
    _attentionContentL4.font = [UIFont systemFontOfSize:textFontSize];
    _attentionContentL4.text = @"4、收取费用或押金的都有欺诈嫌疑，请警惕！";
    _attentionContentL4.numberOfLines = 0;
    _attentionContentL4.adjustsFontSizeToFitWidth = YES;
    return _attentionContentL4;
}
...
- (UIButton *)arrangeWantBtn
{
    ...
    [_arrangeWantBtn setBackgroundImage:[UIImage imageNamed:@"wangArrangeGray"] forState:UIControlStateNormal];
    [_arrangeWantBtn setBackgroundImage:[UIImage imageNamed:@"wangArrangeBlue"] forState:UIControlStateSelected];
    [_arrangeWantBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
    [_arrangeWantBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
...
- (UIButton *)arrangeApplyBtn
{
    ...
    [_arrangeApplyBtn setBackgroundImage:[UIImage imageNamed:@"wangArrangeGray"] forState:UIControlStateNormal];
    [_arrangeApplyBtn setBackgroundImage:[UIImage imageNamed:@"wangArrangeBlue"] forState:UIControlStateSelected];
    [_arrangeApplyBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
    [_arrangeApplyBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
...
- (void)setUpJobDetailWithJob:(StuHomeLocalJob*)job
{
    ...
    [self.jobDetaiView addSubview:self.attentionContentL4];
...
- (void)setAllFrames
{
    if (self.oneJob.isAgent == nil || ![self.oneJob.isAgent isEqualToString:@"1"]) {
        self.jobDescImg.frame = CGRectMake(paddingW, paddingH, titleImgWH, titleImgWH);
        self.jobDescL.frame = CGRectMake(CGRectGetMaxX(self.jobDescImg.frame)+imgTitlePadding, paddingH, screenFrame.size.width-CGRectGetMaxX(self.jobDescImg.frame), titleImgWH);
    }
    else
    {
        self.adviseL.frame = CGRectMake(0, paddingH, screenFrame.size.width, 25);
        [self.jobDetaiView addSubview:self.adviseL];
    ...
    if (self.oneJob.isAgent == nil || ![self.oneJob.isAgent isEqualToString:@"1"])
    {
        ...
        CGFloat attentionContentL4H = [self.attentionContentL4.text boundingRectWithSize:CGSizeMake(labelContentW, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:dict context:nil].size.height;
        self.attentionContentL4.frame = CGRectMake(paddingW, CGRectGetMaxY(self.attentionContentL3.frame)+paddingH, labelContentW, attentionContentL4H);
        
        NSArray *dateArray = self.oneJob.jobDates;
        CGFloat lastBtnY = CGRectGetMaxY(self.attentionContentL4.frame)+20;
        int btnIndex = 0;
        int btnTag = 0;
        int recruitsIndex = 0;
        for (StuJobDateObj *jobDate in dateArray) {
            ...
            NSMutableString *dateStr = [NSMutableString stringWithFormat:@"%@", [jobDateDate substringToIndex:jobDateDate.length-1]];
            NSRange dateRange = [dateStr rangeOfString:@"月"];
            [dateStr replaceCharactersInRange:dateRange withString:@"-"];
            if (![self isValidDateEarlierDate:[NSString stringWithFormat:@"2015-%@", dateStr] BigDate:@""]) {
...
- (void)nonArrangeApplyBtnClick
{
    ...
    if ([responseStr isEqualToString:@"\"success\"" ]) {
        [MBProgressHUD showSuccess:@"职位申请成功"];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else
    {
        [MBProgressHUD showError:@"职位申请失败"];
    }
}
...
- (void)arrangeWantClick:(UIButton*)btn
{
    ...
    for (StuJobDateObj *jobDate in jobDateArray) {
        ...
        //表示被包办,包括自己和别人
        if (jobDate.isArranged!=nil && [jobDate.isArranged isKindOfClass:[NSString class]] && [jobDate.isArranged isEqualToString:@"1"]) {
            if (jobDate.isMyArranged!=nil && [jobDate.isMyArranged isKindOfClass:[NSString class]] && jobDate.isMyArranged.length!=0) {
                //自己包办的
...
- (void)arrangeWantBottomBtnClick
{
    ...
    int randomX = 10;
    StuProfileBBInfo *bbInfo = [StuProfileBBInfo bbJobsWithSessionId:globalSessionId JobId:self.oneJob.jobId];
    NSString *randCommand = bbInfo.command;
    if (randCommand == nil || [randCommand isEqualToString:@""]) {
        randCommand = [NSString stringWithFormat:@"%d%d%d%d%d%d", arc4random()%randomX, arc4random()%randomX, arc4random()%randomX, arc4random()%randomX, arc4random()%randomX, arc4random()%randomX];
    }
    ...
    if ([responseStr isEqualToString:@"\"success\"" ]) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSString stringWithFormat: @"包办成功!\n您的口令是：%@", randCommand] message:@"温馨提示：请务必在2小时内完成包办哦~" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alertView show];
    }
    else
    {
...
//    CGFloat lastBtnY = CGRectGetMaxY(self.arrangeWantBtn.frame);
    ...
    for (StuArrangeRecords *record in recordArray) {
        ...
        for (int i=0; i<arrangeRecordsDetail.recruitsArray.count; i++) {
            ...
            if (arrangeRecordsDetail.recruitsArray[i] != 0) {
                ...
                if (arrangeRecordsDetail.mySignArray!=nil && [arrangeRecordsDetail.mySignArray[i] isKindOfClass:[NSString class]] && arrangeRecordsDetail.mySignArray.count>0 && [arrangeRecordsDetail.mySignArray[i] isEqualToString:@"1"]) {
                    btn.userInteractionEnabled = NO;
                    [btn.arrangeViewImg setImage:[UIImage imageNamed:@"sheng"]];
                }
    ...
    self.jobDetaiView.frame = CGRectMake(0, 25, screenFrame.size.width, CGRectGetMaxY(self.arrangeApplyView.frame)+50);
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (globalSessionId == nil || globalSessionId.length == 0) {
                self.companyInfoModel.isAttentioned = NO;
            }
            else
            {
                self.companyInfoModel.isAttentioned = [StuCompanyInfoModel isAttentionWithCompanyId:companyId SessionId:globalSessionId];
            }
            
            if (self.companyInfoModel.isAttentioned) {
                //[self.companyInfoView.companyAttentionBtn setTitle:@"取消关注" forState:UIControlStateNormal];
                [self.companyInfoView.companyAttentionBtn setSelected:YES];
            }
            else
            {
                //[self.companyInfoView.companyAttentionBtn setTitle:@"关注" forState:UIControlStateNormal];
                [self.companyInfoView.companyAttentionBtn setSelected:NO];
            }
            
            //公司信息按钮以下的整个页面
            self.companyInfoView.frame = CGRectMake(0, paddingH*0.5, screenFrame.size.width, self.jobDetailscrollView.contentSize.height-95);
            //公司信息页面的高度为屏幕大小
            self.jobDetailscrollView.contentSize = CGSizeMake(screenFrame.size.width, screenFrame.size.height);
            [self.companyInfoView setUpCompanyInfoWithCompanyModel:self.companyInfoModel];
            
            [self.companyInfoView.companyAttentionBtn addTarget:self action:@selector(companyAttentionBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)companyAttentionBtnClick:(UIButton*)btn
{
    if (globalSessionId == nil || globalSessionId.length == 0) {
    ...
    NSString *attentionL = [self.companyInfoView.companyBeAttentionL.text substringToIndex:self.companyInfoView.companyBeAttentionL.text.length-1];
    if (btn.selected) {
        [self.companyInfoView.companyBeAttentionL setText:[NSString stringWithFormat: @"%d次", attentionL.intValue-1]];
    }
    else
    {
        [self.companyInfoView.companyBeAttentionL setText:[NSString stringWithFormat: @"%d次", attentionL.intValue+1]];
    ...
    [btn setSelected:!btn.selected];
    [StuCompanyInfoModel companyAttentionWithCompanyId:self.oneJob.companyId MemberId:globalSessionId];
}
...
- (BOOL)isValidDateEarlierDate:(NSString*)earlierDate BigDate:(NSString*)latterDate
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    if (latterDate == nil || latterDate.length == 0)
    {
        latterDate = [dateFormatter stringFromDate:[NSDate date]];
    }
    
    
    NSDate *dt1 = [[NSDate alloc] init];
    NSDate *dt2 = [[NSDate alloc] init];
    dt1 = [dateFormatter dateFromString:earlierDate];
    dt2 = [dateFormatter dateFromString:latterDate];
